...
from GPS_Printer import *
from GPS_Train import *
...
if __name__ == '__main__':
    ...
    train_data_origin = config_input(data_path=param.train_path, item_dict=item_dict, user_dict=user_dict)
    test_data = config_input(data_path=param.test_path, item_dict=item_dict, user_dict=user_dict)
    if param.fast_running:
        train_data = train_data_origin[:int(param.fast_ratio*len(train_data_origin))]
    ...
    input_matrices = trans_matrix_form(train_data_origin)
    ...
    test_batches = generate_batches(input_data=test_data, batch_size=param.batch_size, padding_num=len(item_dict),
                                    is_train=False)
    print("Batches loaded. Initializing EA_GPS network...")

    num_items = len(item_dict)
    num_users = len(user_dict)

    module = EA_GPS(num_items=num_items, num_users=num_users, laplace_list=laplace_list)
